...
class Solution {
    public int minSpeedOnTime(int[] dist, double hour) {
        int l = 1, r = 0, n = dist.length;
        if (n - 1 >= hour) return -1;
        for (int num : dist) if (num > r) r = num;
        r = Math.max(r, (int)(dist[n - 1] / (hour - (n - 1)))) + 1;
        while (l <=r) {
            int mid = l + (r - l) / 2;
            if (!check(dist, mid, hour)) l = mid + 1;
            else r = mid-1;
        }
        return l;
    }
    boolean check(int[] dist, int k, double hour) {
        int sum = 0, len = dist.length;
        for (int i = 0; i < len - 1; ++i) {
            sum += (dist[i] + k - 1) / k;
        }
        return sum + 1.0 * dist[len - 1] / k <= hour;
    }
